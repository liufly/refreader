...
def collate(samples, pad_idx, eos_idx):
    if len(samples) == 0:
        # return {}
        # return an empty dictionary may cause problems for trainer._forward
        # where logging_ouput tries to get 'ntokens' from sample
        # may be better to return a dummy dictionary
        return {
            'id': torch.LongTensor([]),
            'ntokens': 0,
            'net_input': {
                'src_tokens': torch.LongTensor([[pad_idx]]),
                'src_ner': torch.LongTensor([[pad_idx]]),
                'src_chunk': torch.LongTensor([[pad_idx]]),
            },
            'target': torch.LongTensor([[pad_idx]]),
        }
