...
def collate_gates(values, empty_idx):
    """Convert a list of 1d tensors into a padded 3d tensor."""
    for value in values:
        assert values[0].size(1) == value.size(1)
    size = max(v.size(0) for v in values)
    mem_cells = values[0].size(1)
    res = values[0].new(len(values), size, mem_cells).fill_(empty_idx)
...
def collate_corefs(values, empty_idx, move_eos_to_beginning=False):
    """Convert a list of 2d tensors into a padded 3d tensor."""
    for value in values:
        assert value.size(0) == value.size(1)
    size = max(v.size(0) for v in values)
    res = values[0].new(len(values), size, size).fill_(empty_idx)
    res_mask = values[0].new(len(values), size, size).fill_(0)
    ...
    for i, v in enumerate(values):
        # copy_tensor(v, res[i][size - len(v):] if left_pad else res[i][:len(v)])
        res[i, :v.size(0), :v.size(1)].copy_(v)
        res_mask[i, :v.size(0), :v.size(1)].fill_(1)
    return res, res_mask

def collate_bert_weights(values, empty_idx, move_eos_to_beginning=False):
    """Convert a list of 2d tensors into a padded 3d tensor."""
    for value in values:
        assert value.size(0) == 4
        assert value.size(2) == 768
    size = max(v.size(1) for v in values)
    # res = values[0].new(len(values), size, size).fill_(empty_idx)
    res = values[0].new(len(values), 4, size, 768).fill_(empty_idx)
    # res_mask = values[0].new(len(values), size, size).fill_(0)
